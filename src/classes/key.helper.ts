 ...
 */
export function encodeKeys(data: object, keyMap: IKeyMap): Map<string|number, any> {
  const map = new Map();
 ...
 */
export function decodeKeys(data: Map<string|number, any>, keyMap: IKeyMap): object {
  const result = {};
  // If we have a mapping, use it to map the data
