...
describe("Bytewords", () => {
  ...
  const uint8ArrayInput = Uint8Array.from([
    ...
    32, 60, 144, 230, 210, 137, 184, 197, 33, 113, 88, 14, 157, 31, 177, 46, 1,
    115, 205, 69, 225, 150, 65, 235, 58, 144, 65, 240, 133, 69, 113, 247, 63,
    53, 242, 165, 160, 144, 26, 13, 79, 237, 133, 71, 82, 69, 254, 165, 138, 41,
    85, 24,
  ]);

  describe("Encoding to Bytewords", () => {
    test("Standard", () => {
      expect(bytewords.encode(hexInput, bytewords.STYLES.STANDARD)).toBe(
      ...
      expect(
        bytewords.encode(
          uint8ArrayToHex(uint8ArrayInput),
    ...
    test("URI", () => {
      expect(bytewords.encode(hexInput, bytewords.STYLES.URI)).toBe(
        "tuna-acid-draw-oboe-acid-good-slot-axis-limp-lava-brag-holy-door-puff-monk-brag-guru-frog-luau-drop-roof-grim-also-trip-idle-chef-fuel-twin-tied-draw-grim-ramp"
      );
      expect(
        bytewords.encode(uint8ArrayToHex(uint8ArrayInput), bytewords.STYLES.URI)
      ).toBe(
        "yank-toys-bulb-skew-when-warm-free-fair-tent-swan-open-brag-mint-noon-jury-list-view-tiny-brew-note-body-data-webs-what-zinc-bald-join-runs-data-whiz-days-keys-user-diet-news-ruby-whiz-zone-menu-surf-flew-omit-trip-pose-runs-fund-part-even-crux-fern-math-visa-tied-loud-redo-silk-curl-jugs-hard-beta-next-cost-puma-drum-acid-junk-swan-free-very-mint-flap-warm-fact-math-flap-what-limp-free-jugs-yell-fish-epic-whiz-open-numb-math-city-belt-glow-wave-limp-fuel-grim-free-zone-open-love-diet-gyro-cats-fizz-holy-city-puff"
      );
    });
    test("Minimal", () => {
      expect(bytewords.encode(hexInput, bytewords.STYLES.MINIMAL)).toBe(
  ...
  describe("Decoding from Bytewords", () => {
    test("Standard", () => {
      ...
      ).toBe(uint8ArrayToHex(uint8ArrayInput));
    });
    test("URI", () => {
      expect(
        bytewords.decode(
    ...
    test("Invalid checksums", () => {
      ...
      expect(() =>
        bytewords.decode("aeadaolazojendeowf", bytewords.STYLES.MINIMAL)
      ).toThrow(AssertionError);
    });
    ...
    test("Too short", () => {
      ...
      expect(() => bytewords.decode("wolf")).toThrow(AssertionError);

      expect(() => bytewords.decode("")).toThrow(AssertionError);
    });
  });
});
